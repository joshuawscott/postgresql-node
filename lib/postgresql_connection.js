...
function PostgresqlConnection(opts) {
  ...
  self.sendPasswordMessage = function (password, encrypted) {
    self.connection.write(generator.passwordMessage(password, encrypted));
  };
  ...
  self.sendQuery = self.query;

  self.authenticate = function () {
    self.connection.write(generator.startupMessage(self.username, self.database));
  };
  self.sendStartupMessage = self.authenticate;

  self.on('authenticationCleartextPassword', function (messageObj) {
    self.connection.write(generator.passwordMessage(self.password));
  });
  self.on('authenticationMD5Password', function (messageObj) {
    self.salt = messageObj.salt;
    self.connection.write(generator.passwordMessage(self.password, true, self.username, self.salt))
  });
