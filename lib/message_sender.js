...
//  var messageString = 'B',
//    messageLength,
//    destinationPortal = options.destinationPortal + '\0',
//    sourcePreparedStatement = options.sourcePreparedStatement + '\0',
...
exports.close = function(type, name) {
  ...
  var buf = new Buffer(messageLength + 1);
  buf.write('C',0);
...
var createMessageBuffer = function (arr) {
  ...
  for (var i=0; i<arr.length; i++) {
    ...
    val = arr[i][type];
    if (type == 'string') {
      if (typeof val !== 'string') throw 'Type mismatch, expected string as "string", got ' + typeof val;
      bufferLength += Buffer.byteLength(val) + 1;

    } else if (type == 'int32') {
      if (typeof val !== 'number') throw 'Type mismatch, expected int32 as "number", got ' + typeof val;
      bufferLength += 4;

    } else if (type == 'int16') {
      if (typeof val !== 'number') throw 'Type mismatch, expected int16 as "number", got ' + typeof val;
      bufferLength += 2;

    } else if (type == 'byte') {
      if (typeof val !== 'string') throw 'Type mismatch, expected byte as "string", got ' + typeof val;
      bufferLength += 1;
  ...
  console.log(bufferLength.toString());

  var messageBuffer = new Buffer(bufferLength);
  var offset = 0
  for (var i=0; i<arr.length; i++) {
    type = Object.keys(arr[i])[0].toLowerCase();
    val = arr[i][type];
    if (type == 'string') {
      ...
      messageBuffer.write(val + '\0', offset);
